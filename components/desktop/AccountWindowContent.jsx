    ...
    return (
      <div className="h-full bg-[linear-gradient(180deg,rgba(10,14,25,0.98),rgba(7,9,16,1))] px-6 py-6 text-white">
        ...
        <p className="mt-2 max-w-[520px] text-[14px] leading-7 text-white/68">
          Inicia sesión con Google para guardar perfil, créditos, claves API y configuración de cuenta por usuario.
        </p>

        <div className="mt-6 rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
          <p className="text-[13px] text-white/75">
            El acceso usa Google Login. Si aún no funciona en tu entorno, revisa GOOGLE_CLIENT_ID y GOOGLE_CLIENT_SECRET.
          </p>
          <button
            type="button"
            onClick={onLogin}
    ...
    <div className="h-full bg-[linear-gradient(180deg,rgba(10,14,25,0.98),rgba(7,9,16,1))] px-6 py-6 text-white">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <section className="rounded-[26px] border border-white/10 bg-white/[0.04] p-5">
          <p className="text-[11px] uppercase tracking-[0.18em] text-cyan-300/85">Uso y créditos</p>
          <div className="mt-4 space-y-3">
            <MetricRow label="Crédito disponible" value={`$${account.creditsUsd.toFixed(2)}`} />
            <MetricRow label="Consumo acumulado" value={`$${account.totalUsageUsd.toFixed(2)}`} />
        ...
        <section className="rounded-[26px] border border-white/10 bg-white/[0.04] p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <p className="text-[12px] leading-6 text-white/48">
              Este bloque ya queda preparado para billing, suscripciones, límites y upgrades SaaS más adelante.
            </p>
          </div>
        </section>
        ...
        <section className="rounded-[26px] border border-white/10 bg-white/[0.04] p-5">
          <p className="text-[11px] uppercase tracking-[0.18em] text-amber-300/85">Configuración básica</p>
          <div className="mt-4 space-y-4">
            <label className="block">
              ...
              <input
                ...
                placeholder="Cómo quieres aparecer en la plataforma"
            ...
            <label className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/15 px-4 py-3">
              <span className="text-[13px] text-white/78">Avisarme cuando el crédito baje</span>
              <input
                type="checkbox"
